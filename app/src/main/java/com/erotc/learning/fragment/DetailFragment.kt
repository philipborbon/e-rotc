...
class DetailFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dictionaryId = arguments?.getLong(DICTIONARY_ID) ?: 0
    ...
    private fun boldText(text: String?, sentence: String?): SpannableStringBuilder {
        ...
        val index = comparableSentence?.indexOf(comparableText ?: "") ?: 0
        if (index != -1) {
            spannableStringBuilder.setSpan(StyleSpan(Typeface.BOLD), index, index + (comparableText?.length ?: 0), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        }
    ...
    companion object {
        ...
        fun newInstance(dictionaryId: Long): DetailFragment {
            val fragment = DetailFragment()
            val bundle = Bundle()
            bundle.putLong(DICTIONARY_ID, dictionaryId)
